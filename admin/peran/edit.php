<?php
  include "../../config.php";
  $query = mysqli_query($conn, "SELECT * FROM detail_aktor JOIN film ON detail_aktor.id_film=film.id_film");
              ...
              <div class="card-body">
                ...
                <div class="col-md-12">
                  <div class="form-group">
                    ...
                    if(isset($_GET['id_film'])){
                      $id_film = $_GET['id_film'];
                      $aktor_peran = "SELECT id_aktor FROM detail_aktor WHERE id_film = '$id_film'";
                      $peran = mysqli_query($conn, $aktor_peran);

                      $peran_array = [];
                      foreach($peran as $rowperan){
                        $peran_array[] = $rowperan['id_aktor'];
                      }
                    }
                    ?>
                    ...
                    if(mysqli_num_rows($query_run) > 0){
                      foreach($query_run as $row){
                        ?>
                          <option value="<?php echo $row['id_aktor']; ?>"
                            <?php echo in_array($row['id_aktor'], $peran_array) ? 'selected' : '' ?>
                          >
